...
public:
    ...
    void DrawLine(int x0, int y0, int x1, int y1, ALLEGRO_COLOR color)
    {
        ...
        { // Jeœli nachylenie jest mniejsze ni¿ 1 (dominuj¹ca oœ X)
            ...
            y = y0;
            for (x = x0; x <= x1; x++) {
                al_put_pixel(x, (int)(y + 0.5), color); // Rysowanie pikseli
                y += m;
            }
        }
        else { // Jeœli nachylenie jest wiêksze ni¿ 1 (dominuj¹ca oœ Y)
            if (y0 > y1) { // Jeœli zaczynamy od do³u, zamieniamy punkty miejscami
                swap(x0, x1);
                swap(y0, y1);
            }
    ...
    // Funkcja do rysowania wype³nionego trójk¹ta (metoda skan-linii) - coœ nie dzia³a
    ...
    void BorderFill(int x, int y, ALLEGRO_COLOR fillColor, ALLEGRO_COLOR borderColor)
    {
        ...
        std::cout << "Obszar wypelniony (border fill)\n";
    }
    ...
    void FloodFill(int x, int y, ALLEGRO_COLOR fillColor)
    {
        ...
        std::cout << "Obszar wypelniony (flood fill)\n";
    }
...
int main()
{
    ...
    Point2D endPoint(700, 450, engine.blue);
    ...
    // zrób wype³nione figury
    rysuj.DrawFilledRectangle(200, 200, 300, 300, blue);
    rysuj.DrawFilledTriangle(50, 50, 100, 100, 50, 300, white);
    rysuj.DrawFilledCircle(300, 300, 150, green);
    Sleep(1000);

    // fllod fill
    rysuj.DrawFilledRectangle(700, 500, 800, 600, red);
    rysuj.FloodFill(750, 550, white);
